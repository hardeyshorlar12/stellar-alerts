...
> {
  try {
    ...
    console.log(`[Registry] Loaded ${contractRegistry.size} active contracts (${subscriptions.length} subscriptions)`);
...
export function routeEventToUsers(
  ...
  let matcheeUserIds = contract.topicRoutes.get(topic);
